...
(define (handle-cond cond-clause env)
  (cond ((eq? cond-clause '()) (cond))
		((eq? (caar cond-clause) 'else) (cond-body (cdar cond-clause) env))
		((my-eval (caar cond-clause) env) (cond-body (cdar cond-clause) env))
		(else (handle-cond (cdr cond-clause) env))
		))

(define (cond-body body env)
  (cond ((not (pair? (cdr body))) (my-eval (car body) env))
		(else (cond-body (cdr body) env))
		))
